...
import { fetchOutreachLogs, type OutreachLogsResponse } from '@/lib/leadFunnelApi';

/** Render an outreach timestamp as German local time (Europe/Berlin).
 *
 * The backend stores `sentAt` via `datetime.utcnow()` and serialises it with no
 * timezone suffix, so an offset-less string must be pinned to UTC — left as-is
 * the browser would read it as local time and shift the clock by the viewer's
 * own offset.
 */
function germanTime(iso: string | null | undefined): string {
  if (!iso) return '';
...
/* ── skeleton loader ──
 * Mirrors the real row/card geometry so the panel doesn't reflow when the first
 * response lands. Matches the shimmer convention in LinkedInBoardLive.
 */
const shimmer: React.CSSProperties = {
  background: 'linear-gradient(90deg, var(--color-surface) 25%, var(--color-border) 37%, var(--color-surface) 63%)',
  backgroundSize: '400% 100%', animation: 'outreachSkeleton 1.4s ease infinite',
};
...
/** Placeholder for one summary tile — the real card is a label over a big count. */
...
/** Placeholder for one outreach log row. */
...
export function ImportOutreachStatusPanel({ open, onClose, runId, runName }: {
  open: boolean; onClose: () => void; runId: string; runName: string;
}) {
  const [data, setData] = React.useState<OutreachLogsResponse | null>(null);
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);

  // Poll by chaining a timeout off each completed response rather than on a
  // fixed interval: a slow response used to let the next tick fire anyway, so
  // requests stacked until they filled the browser's ~6-connection budget for
  // this origin and starved every other call on the page.
  React.useEffect(() => {
    if (!open) return;
    let stop = false;
    ...
    const load = async () => {
      try {
        const d = await fetchOutreachLogs(runId);
        if (stop) return;
        setData(d);
        setError(null);
      } catch (e) {
    ...
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 'min(92vw, 480px)', background: 'var(--color-bg)', zIndex: 1101, boxShadow: 'var(--shadow-lg)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
          {loading && <style>{`@keyframes outreachSkeleton{0%{background-position:100% 50%}100%{background-position:0 50%}}`}</style>}
          ...
          {/* The tiles read as authoritative counts, so they must not show 0
              before the first response — that's indistinguishable from a run
              that genuinely has no outreach. */}
              ...
              <div style={{ marginBottom: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '12px', fontWeight: 600, color: 'var(--color-text-2)' }}>
                    <span className="outreach-status-spinner" style={{ width: 10, height: 10, borderRadius: '50%', border: '1.5px solid var(--color-brand)', borderTopColor: 'transparent', display: 'inline-block' }} />
                    Sending outreach…
                  </span>
                ...
                <style>{`@keyframes outreach-status-spin{to{transform:rotate(360deg)}}.outreach-status-spinner{animation:outreach-status-spin .7s linear infinite;}`}</style>
              </div>
            );
          })()}
          ...
          <div style={{ border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', overflow: 'hidden' }}>
            {loading ? (
              ...
              const [bg, c] = STATUS[r.status] ?? STATUS.skipped;
              return (
                <div key={r._id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', padding: '11px 14px', borderBottom: '1px solid var(--color-border)' }}>
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '11.5px', color: 'var(--color-brand)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.prospectName} <span style={{ color: 'var(--color-text-3)' }}>@ {r.prospectCompany}</span></div>
                  </div>
                  <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                    <span style={{ fontSize: '10.5px', fontWeight: 700, textTransform: 'capitalize', background: bg, color: c, padding: '3px 9px', borderRadius: 'var(--radius-full)' }}>{r.status}</span>
                    {germanTime(r.updatedAt) && (
                      <span
                        title={`${r.status === 'pending' ? 'Queued' : 'Sent'} at ${germanTime(r.updatedAt)} (Europe/Berlin)`}
  ...
  return (
    <div style={{ background: bg, borderRadius: 'var(--radius-md)', padding: '12px 14px', boxShadow: live ? `inset 0 0 0 1.5px ${c}` : 'none' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
        {live && <span className="outreach-pending-pulse" style={{ width: 6, height: 6, borderRadius: '50%', background: c, display: 'inline-block' }} />}
      ...
      {live && <style>{`@keyframes outreach-pending-pulse{0%,100%{opacity:1}50%{opacity:.25}}.outreach-pending-pulse{animation:outreach-pending-pulse 1s ease-in-out infinite;}`}</style>}
    </div>
  );
}
